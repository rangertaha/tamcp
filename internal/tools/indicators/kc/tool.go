// Package kc registers Keltner Channels (Pandas TA, cinar).
package kc
...
func init() {
	talib.RegisterEntry(&talib.Entry{
		Name:        "kc",
		Description: "Keltner Channels: EMA(close, p) ± mult * ATR(h,l,c, p). Returns upper, middle, lower.",
		Group:       "volatility",
		Params: []talib.Param{
		...
		Run: func(args map[string]any) (any, string, error) {
			...
			u, mid, lo := talib.KCFn(h, l, c, p, m)
			return talib.Three(u, mid, lo, [3]string{"upper", "middle", "lower"}), talib.Tersum("kc", mid), nil
		},
	})
}
